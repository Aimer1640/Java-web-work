        ...
        <div class="am-fr" style="width: 180px;">
          <div class="am-input-group am-input-group-sm">
            ...
            <span class="am-input-group-btn" style="width: 30%;">
				        <a href="${basePath}/project/addProject"><button id="searchTeacher" class="am-btn am-btn-default" type="button">添加</button></a>
			</span>
          </div>
        </div>
      </div>
    ...
    <div class="am-panel am-panel-default">
      <div class="am-panel-bd am-scrollable-horizontal">
        <table class="am-table am-table-centered am-table-striped am-table-hover">
          ...
          <tbody id="chooseBody">
          <c:forEach items="${projectList}" var="p" varStatus="s">
            <form name='form'>
              <input type="hidden" name="id" value="${p.projectId}" />
              <tr>
                <th scope="row">${s.index+1 }</th>
                <td><a href='project/findProjectById?id=${p.projectId}'>
                    ${p.projectId }</a></td>
                <td>${p.projectName }</td>
                <td>${p.projectTeacher}</td>
                <td>${p.projectDemand}</td>
                <td>${p.projectState}</td>
                <td>${p.projectFile}</td>
                <td><a href="${basePath}/project/findProjectByProjectId/${p.projectId}">修改</a>
                  <a onclick="return confirm('确定要删除吗？');" href="${basePath}/project/deleteProjectById/${p.projectId}">删除</a> </td>
<%--                <!-- 点击下载图片 -->--%>
