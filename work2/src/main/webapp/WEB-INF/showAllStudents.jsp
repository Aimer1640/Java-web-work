          ...
          <c:forEach items="${studentList}" var="stu" varStatus="s">
            <form name='form' action='showUpdateInfoServlet'>
              ...
              <tr>
                ...
                <td>${stu.studentId}</td>
                <td><a href='findAllStudents?id=${stu.studentId}'>
                    ${stu.studentName }</a></td>
                <td>${stu.majorId }</td>
                <td>${stu.deptId}</td>
                <td>${stu.absence}</td>

                <td><a href="${basePath}/student/findStudentByStudentId/${stu.studentId}">修改</a>
                  <a onclick="return confirm('确定要删除吗？');" href="${basePath}/student/deleteStudentById/${stu.studentId}">删除</a> </td>
<%--                <!-- 点击下载图片 -->--%>
